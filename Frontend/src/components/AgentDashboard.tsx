...
import {
  ...
  BarChart2, Zap, Clock, Globe, Cpu, CheckCircle2, XCircle, ChevronRight
} from 'lucide-react';
import VerifyModal, { type VerifyAgentData } from './VerifyModal';
import { config } from '../config';
import { SuccessIcon, UptimeIcon, FlaggedIcon, AlertIcon } from './AegisIcons';

/* This is the same dashboard rendered at /agent/[address] — extracted into
   a reusable component (parameterized by `address` instead of reading it
   from the router) so it can also be shown inline on the pitch page, as
   concrete proof the "performance analytics dashboard" already exists,
   without embedding the real page in an iframe. */
...
export default function AgentDashboard({
  ...
  // For locally-run demo agents (from the /developer CLI Walkthrough), read
  // name from localStorage and build a meta-like object.
  ...
  const meta = AGENT_META[address] ?? demoMeta;
  ...
  useEffect(() => {
    ...
    if (isDemo) {
      ...
      setRep(demoData);
      if (demoData) setChartData(buildChartData(demoData));
      setLoading(false);
      return;
    }
    ...
    fetchReputation(address).then(data => {
      ...
      setRep(data);
      if (data) setChartData(buildChartData(data));
      setLoading(false);
    });
  }, [address, isDemo]);
              ...
              {(['success', 'volume', 'executions'] as Tab[]).map(t => (
                <button
                  ...
                  className={`px-3 py-1 rounded-full text-[11px] font-medium transition-all ${
                    tab === t ? 'bg-cyan-primary text-bg-base font-bold' : 'text-text-muted hover:text-text-secondary'
                  }`}
                >
